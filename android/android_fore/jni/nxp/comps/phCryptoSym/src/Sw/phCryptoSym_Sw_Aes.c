    ...
    )
{
    ...
    uint8_t PH_MEMLOC_BUF * pKeyPtrNm1 = &pDataParams->pKey[((bNkCurrent) << 2) - 4];
    uint8_t PH_MEMLOC_BUF * pCurKeyPtr = &pDataParams->pKey[(bNkCurrent) << 2];

    /* First copy the key provided... */
    memcpy(pDataParams->pKey, pKey, bNkCurrent << 2); /* PRQA S 3200 */
    ...
    */
    ...
    for (i= bNkCurrent; i < bNkMax; i++)
    {
        ...
        }else if ((bNkCurrent == 8) && ((i % bNkCurrent) == 4))
        {
...
{    
    ...
    /* Recopy the key */
    memcpy(bTmpKey, pDataParams->pKey, bNk << 2); /* PRQA S 3200 */
...
void phCryptoSym_Sw_Aes_AddRoundKey(uint8_t PH_CRYTOSYM_SW_FAST_RAM * pState, uint8_t PH_CRYTOSYM_SW_FAST_RAM * pKey, uint8_t * pNkCurrent, uint8_t bNk, uint8_t bMode)
{
    ...
    for (i=0;i<4;i++)
    {
        ...
        pCurKeyPtr = &pKey[bModulus << 2];

        /* In case of decryption, xoring has to be done upfront. */
        if (bMode == PH_CRYPTOSYM_KEYSCHEDULE_DECRYPTION)
        {
            pState[(3-i)*4] ^= *(pCurKeyPtr + 0);
            pState[(3-i)*4+1] ^= *(pCurKeyPtr + 1);
            pState[(3-i)*4+2] ^= *(pCurKeyPtr + 2);
            pState[(3-i)*4+3] ^= *(pCurKeyPtr + 3);
        }
        ...
        }else
        {
            if ( bModulus == 0)
            {
                ...
                bOffset = (uint8_t)(((uint8_t)((uint8_t)bNk-1)<<2) + 1);
                /* now calculate the following calculation: */
                /* w[i] = w[i-Nk] xor SubWord(RotWord(w[i-1])) xor Rcon[i/Nk] */
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr+bOffset)] ^ phCryptoSym_Sw_Aes_Rcon[(*pNkCurrent)/bNk - 1];
                pCurKeyPtr++;
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr+bOffset)];
                pCurKeyPtr++;
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr+bOffset)];
                pCurKeyPtr++;
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr+bOffset - 4)];       
                pCurKeyPtr++;
            }else if ((bNk == 8) && ((*pNkCurrent % bNk) == 4))
            {
                /* w[i] = w[i-Nk] xor SubWord(w[i-1]) */
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr-4)];
                pCurKeyPtr++;
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr-4)];
                pCurKeyPtr++;
                *pCurKeyPtr ^= phCryptoSym_Sw_Aes_sboxTable[*(pCurKeyPtr-4)];
        ...
        {
            pState[i*4] ^= *(pCurKeyPtr - 4);
            pState[i*4+1] ^= *(pCurKeyPtr - 3);
            pState[i*4+2] ^= *(pCurKeyPtr - 2);
            pState[i*4+3] ^= *(pCurKeyPtr - 1);
        }
    }
}
#endif /* PH_CRYPTOSYM_SW_ONLINE_KEYSCHEDULING */
...
void phCryptoSym_Sw_Aes_AddRoundKey(uint8_t PH_CRYTOSYM_SW_FAST_RAM * pState, uint8_t PH_MEMLOC_BUF * pKey, uint8_t bCnt)
{
    ...
    uint8_t PH_MEMLOC_BUF * pRoundKey = &pKey[bCnt << 4];
